...
```bash
aws lambda create-function --function-name vat-validator-graal \
...
```bash
aws lambda invoke --function-name vat-validator-graal --payload '{"resource": "/{proxy+}", "path": "/ping", "httpMethod": "GET"}' build/response.txt
