...
[source,java]
.vat-validator-graal/build.gradle
----
include::{sourceDir}/vat-validator-graal/build.gradle[tag=http-client]
----

Create a `VatService` which consumes the SOAP Service:

[source,java]
.vat-validator-graal/src/main/java/example/micronaut/VatService.java
----
include::{sourceDir}/vat-validator-graal/src/main/java/example/micronaut/VatService.java[]
----
